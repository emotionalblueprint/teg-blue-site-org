      ...
      >
        ...
        <article>
          ...
          <section id="two-pathways" aria-labelledby="heading-paths" style={{ marginBottom: 48 }}>
            ...
            <p style={proseStyle}>Not all activation resolves through the same pathway. The signal{"'"}s content — what the nervous system detected (<Link href="/model/m1-emotions-as-signals" style={linkStyle}>M1</Link>) — determines which form of restoration the body requires.</p>
            <h3 style={conceptHeadingStyle}>The Somatic Restoration Pathway</h3>
            <p style={proseStyle}>Somatic emotions — those whose signal content is about the body{"'"}s own state (threat, boundary violation, demand-resource mismatch, contamination, safety confirmed) — can complete through the body{"'"}s own channels: breathing, movement, time, stillness, crying, sleep. The nervous system runs the restoration sequence without requiring input from another person. Stress hormones metabolise, muscles release, the HPA axis stands down, and the body returns toward physiological baseline.</p>
            <h3 style={conceptHeadingStyle}>The Relational Restoration Pathway</h3>
            <p style={proseStyle}>Relational emotions — those whose signal content is about belonging, connection, or the state of the bond — cannot complete through the body{"'"}s own channels. The restoration process does not require somatic discharge. It requires relational evidence — the presence of another person who provides what the signal content requires. No amount of breathing resolves shame. The restoration sequence requires another person to stay — to remain present without contempt after seeing the thing that feels shameful. That staying is the biological signal the restoration pathway needs.</p>
            <h3 style={conceptHeadingStyle}>The Pathway Must Match the Content</h3>
            <p style={proseStyle}>This is a structural constraint of the signal system, not a preference. A person who attempts somatic restoration for relational content — exercising to clear shame, breathing techniques to process grief — produces discharge but does not complete the restoration sequence. The activation remains. The body carries it forward. Many failed attempts at regulation are actually pathway mismatches: the system is being given a somatic intervention for a relational burden, or a relational need is being approached as if it were only bodily activation.</p>
            <h3 style={conceptHeadingStyle}>Developmental Loss of Restoration Pathways</h3>
            <p style={proseStyle}>When relational emotions repeatedly arise in environments where no co-regulating response is available, the person may not develop robust pathways for resolving those states. The child does not fail to develop {"\u201C"}regulation{"\u201D"} in general — they fail to develop the restoration pathway for the specific emotions that require relational evidence. This specificity explains why someone may regulate threat effectively in some contexts (somatic pathway intact) while remaining highly vulnerable to shame, abandonment signals, or relational loss in others (relational pathway never built).</p>
            <p style={proseStyle}>When the pathways never build, the system has no exit. The activation accumulates, the baseline elevates, and the window between resting activation and shutdown narrows. The nervous system stopped generating those signals because no one was receiving them. The physiological endpoint is dorsal shutdown — where resting activation has risen so high that shutdown is the only remaining response.</p>
            <div style={expandableRowStyle}>
              <ExpandableSection title="Established Research" type="opendata"><p style={expandedProseStyle}>Porges (2011) — co-regulation as the mammalian primary regulation pathway. Schore (2003) — right-brain relational regulation in development, shame as primary regulatory affect. Bowlby (1969) — attachment as the relational regulation system. Levine (1997) — the activation cycle as a somatic process requiring completion. Tronick (2007) — mutual regulation model, chronic misattunement. van der Kolk (2014) — developmental consequences of emotional neglect.</p></ExpandableSection>
