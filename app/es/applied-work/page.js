...
const faqItems = [
  ["¿Se puede licenciar el TEG-Blue Engine?", "El marco público y la lógica privada del Engine tienen derechos distintos. El uso de la lógica del Engine, herramientas a medida, integraciones de producto e implementaciones institucionales requiere permiso escrito o un acuerdo independiente."],
  ["¿Puede TEG-Blue construir una aplicación a medida?", "Posiblemente. El primer paso es definir la audiencia, el propósito, la evidencia, las revisiones necesarias y los derechos. Una conversación no crea por sí sola una colaboración, licencia o compromiso de desarrollo."],
  ["¿Puede una organización reutilizar el marco público?", "El contenido público escrito del marco puede reutilizarse bajo CC BY 4.0 con atribución, salvo que una página indique otra cosa. Las marcas, herramientas, código, lógica del Engine, superficies de producto y materiales de terceros quedan excluidos."],
  ["¿El Engine diagnostica o clasifica personas?", "No. Estructura material fuente y lógica para herramientas. No diagnostica, prueba motivos, determina el estado interno de una persona ni convierte una respuesta en un veredicto."],
  ["¿Cómo se tratan la autoría y el material confidencial?", "La autoría, atribución, usos permitidos, acceso a materiales y confidencialidad deben definirse para cada proyecto antes de comenzar el trabajo sustantivo."],
];
...
export default function SpanishAppliedWorkPage() {
  const pageJsonLd = { "@context": "https://schema.org", "@type": "WebPage", name: "Trabajo aplicado con TEG-Blue", url: "https://teg-blue.org/es/applied-work", inLanguage: "es", description: metadata.description, isPartOf: { "@type": "ResearchProject", name: "TEG-Blue: The Emotional Gradient Blueprint", url: "https://teg-blue.org" }, creator: { "@type": "Person", name: "Anna Paretas-Artacho", url: "https://annaparetas.com", sameAs: "https://orcid.org/0009-0005-2394-7162" }, about: { "@type": "Thing", name: "Herramientas aplicadas de TEG-Blue", description: "Aplicaciones educativas y digitales con un alcance definido para una audiencia y una pregunta humana concretas." } };
  ...
  return <div style={{ minHeight: "100vh", background: BG.page, fontFamily: FONT.display }}>
    ...
    <PageLayout header={<ResearcherHero badge="TRABAJO APLICADO" title="Construir una herramienta para una pregunta humana concreta" subtitle="Trabajo aplicado con TEG-Blue" description="Algunos patrones son difíciles de explicar sin perder el contexto. TEG-Blue puede ayudar a convertir una pregunta definida en una explicación visual, página de conducta, recurso educativo o herramienta digital." />}>
      ...
      <section style={{ marginBottom: 58 }}><p style={{ color: SPECTRUM.indigo, fontFamily: FONT.mono, fontSize: 12, fontWeight: 700 }}>PREGUNTAS PRÁCTICAS</p><h2 style={heading}>Lo que una organización puede necesitar saber primero.</h2><div style={{ display: "grid", gap: 10 }}>{faqItems.map(([question, answer]) => <details key={question} style={card}><summary style={{ cursor: "pointer", color: TEXT.primary, fontSize: 17, fontWeight: 650 }}>{question}</summary><p style={{ ...body, marginTop: 12 }}>{answer}</p></details>)}</div></section>
    ...
    </PageLayout><SiteFooter locale="es" /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(pageJsonLd) }} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd) }} />
  </div>;
}
